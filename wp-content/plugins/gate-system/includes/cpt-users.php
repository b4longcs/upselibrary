...
add_action('save_post_gs_user', function ($post_id) {
    ...
    // Prevent saving if barcode is duplicated
    if (isset($_POST['gs_barcode'])) {
        $barcode = sanitize_text_field($_POST['gs_barcode']);
        ...
        if (!empty($barcode)) {
            ...
            if ($existing->have_posts()) {
                ...
                wp_die('<h1>Error: Duplicate Barcode</h1><p>A user with the barcode <strong>' . esc_html($barcode) . '</strong> already exists.</p><p><a href="' . esc_url(admin_url('edit.php?post_type=gs_user')) . '">Go back to users</a></p>', 'Duplicate Barcode', [
                    'response' => 409,
                    'back_link' => true
                ]);
            }
...
add_action('wp_ajax_gs_save_user_meta', function () {
    ...
    if (empty($fields) || !is_array($fields)) {
        wp_send_json_error(['message' => 'Invalid field data.']);
    }

    $allowed_fields = ['name', 'college', 'course', 'barcode', 'type'];

    // ✅ Validate barcode is required
    $barcode = isset($fields['barcode']) ? sanitize_text_field($fields['barcode']) : '';
    if (empty($barcode)) {
        wp_send_json_error(['message' => 'Barcode is required.']);
    }
    ...
    if ($existing->have_posts()) {
        $duplicate_id = $existing->posts[0];
        $duplicate_title = get_the_title($duplicate_id);

        wp_send_json_error([
            'message' => 'Barcode already exists.',
            'duplicate_id' => $duplicate_id,
            'duplicate_name' => $duplicate_title,
        ]);
    }
    ...
    if (!$post_id) {
        if (empty($fields['name'])) {
            wp_send_json_error(['message' => 'Name is required to create a new user.']);
        }
        ...
        if (is_wp_error($post_id)) {
            wp_send_json_error(['message' => 'Failed to create new user.']);
        }
    } else {
        // ✅ Update existing post title if name provided
        if (!empty($fields['name'])) {
            wp_update_post([
                'ID'         => $post_id,
                'post_title' => sanitize_text_field($fields['name']),
